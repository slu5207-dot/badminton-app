...
const App: React.FC = () => {
  ...
  const handleFinishMatch = (courtId: number, scoreA: number, scoreB: number, explicitWinner?: 'Team A' | 'Team B' | 'Draw') => {
    ...
    requests.forEach(req => {
        ...
        const c = nextCourts.find(ct => ct.id === req.cid); if (c && c.status === 'empty') { c.players[req.idx] = req.p; c.nextMatch[req.idx] = null; busyIds.add(req.p.id); modifiedCourtIds.add(c.id); }
    });
        ...
        <div className="max-w-7xl mx-auto px-2 md:px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-black/20 p-1 rounded-lg">
               <button onClick={() => setActiveTab('signup')} className={`px-3 md:px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'signup' ? 'bg-emerald-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>報名</button>
               <button onClick={() => setActiveTab('manage')} className={`px-3 md:px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'manage' ? 'bg-emerald-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>排點</button>
            </div>
            <div className="relative">
              ...
              {showMenu && (<div className="absolute right-0 top-full mt-2 w-48 bg-[#0a2e1f] border border-white/10 rounded-xl shadow-xl z-50 flex flex-col overflow-hidden animate-fade-in"><button onClick={() => { handleClearCourts(); setShowMenu(false); }} className="px-4 py-3 text-left text-sm hover:bg-white/5 flex items-center gap-2 text-orange-300"><LogOut size={16} /> 全部下場</button><button onClick={() => { handleResetPlayCounts(); setShowMenu(false); }} className="px-4 py-3 text-left text-sm hover:bg-white/5 flex items-center gap-2"><RotateCcw size={16} /> 重置場次</button><div className="h-px bg-white/10 my-0"></div><button onClick={() => { updateData({ players: [], courts: INITIAL_COURTS, history: [] }); setShowMenu(false); }} className="px-4 py-3 text-left text-sm text-red-400 hover:bg-white/5 flex items-center gap-2"><Trash2 size={16} /> 全部清空</button></div>)}
            </div>
          </div>
        </div>
      </header>

      {/* 修改 2: 外層 padding 縮小，以利手機顯示 */}
      <main className="flex-1 max-w-7xl mx-auto w-full p-2 md:p-4 overflow-hidden flex flex-col relative pb-10">
            ...
            <div className="flex-1 flex flex-col gap-4 overflow-y-auto pb-20 lg:pb-0 custom-scrollbar">
              ...
              <div className="flex flex-wrap gap-2 items-center bg-[#0a2e1f] p-3 md:p-4 rounded-xl border border-white/10">
                <div className="flex-1 min-w-[200px] flex items-center gap-2">
                   <h3 className="text-gray-400 text-sm font-bold flex items-center gap-1 hidden md:flex"><Settings size={16} /> 操作</h3>
                   ...
                   <div className="flex md:hidden gap-1">
                      <button onClick={handleAutoAssign} className="p-2 bg-emerald-600 rounded text-white"><Zap size={16}/></button>
                      <button onClick={() => setShowHistoryMobile(true)} className="p-2 bg-gray-700 rounded text-white relative"><HistoryIcon size={16}/><span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full animate-pulse"></span></button>
                   </div>
                </div>
                {/* Desktop Buttons */}
                <div className="hidden md:flex items-center gap-2 w-auto">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <button onClick={handleResetPlayCounts} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-2 rounded-lg font-medium text-xs whitespace-nowrap">重置</button>
                  </div>
