...
const SignupSystem: React.FC<Props> = ({ onImportToGame }) => {
  ...
  // --- 🔴 修復完成的編輯載入功能 ---
  // 這兩個函數會準確地將資料寫入 State，不會再「點了沒反應」

  // 1. 編輯已報名者
  const handleEditParticipant = (name: string, details: ParticipantDetail) => {
      setEditingTarget(name); // 標記為正在編輯這個人
      setInputName(name);
      setInputLevel(details.level);
      setInputBP(details.battlePower.toString());
      window.scrollTo({ top: 0, behavior: 'smooth' }); // 自動滾動到上方
  };
  
  // 2. 載入常用名單 (新增模式)
  const handleLoadFavorite = (fav: FavoritePlayer) => {
      setEditingTarget(null); // 清除編輯狀態 (這是新增)
      setInputName(fav.name);
      setInputLevel(fav.level);
      setInputBP(fav.battlePower.toString());
      setActiveTab('edit'); // 自動切到編輯頁
  ...
  const handleSubmitParticipant = () => {
    ...
    if (editingTarget) {
        if (name !== editingTarget) {
            if (evt.participants.includes(name) || evt.waitlist.includes(name)) {
                alert("新名字已存在，無法更名！");
                return;
            }
            evt.participants = evt.participants.map(p => p === editingTarget ? name : p);
            evt.waitlist = evt.waitlist.map(p => p === editingTarget ? name : p);
            delete details[editingTarget];
        }
        ...
        if (evt.participants.includes(name) || evt.waitlist.includes(name)) {
            // 這裡也用 inputLevel/inputBP
             if (window.confirm(`${name} 已經報名了，要直接更新他的資料嗎？`)) {
                 details[name] = { level: inputLevel, battlePower: bp };
                 evt.participantDetails = details;
                 updateEvent(evt);
                 setInputName('');
    ...
    <div className="flex flex-col h-full bg-[#031811] text-white relative">
       ...
       <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
          ...
          {activeTab === 'edit' && (
              <>
                 <div className="bg-[#0a2e1f] p-4 rounded-xl border border-white/10 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div>
                           ...
                           {isAddingLocation ? (
                               ...
                               <div className="relative">
                                   ...
                                   <button onClick={() => handleDeleteLocation(currentEvent.location)} className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-500 hover:text-red-400 p-1" title="刪除此地點"><Trash2 size={12}/></button>
                       ...
                       <div>
                           ...
                           <div className="relative">
                               <select value={currentEvent.startTime} onChange={e => updateEvent({...currentEvent, startTime: e.target.value})} className="w-full bg-[#031811] border border-white/20 rounded p-2 text-sm appearance-none focus:border-emerald-500 outline-none block">{TIME_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}</select>
                       ...
                       <div>
                           ...
                           <div className="relative">
                               <select value={currentEvent.endTime} onChange={e => updateEvent({...currentEvent, endTime: e.target.value})} className="w-full bg-[#031811] border border-white/20 rounded p-2 text-sm appearance-none focus:border-emerald-500 outline-none block">{TIME_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}</select>
                               <Clock size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none"/>
                           </div>
                       </div>
                    </div>
                 ...
                 {/* 輸入區塊 */}
                 <div className={`flex flex-col gap-2 p-3 rounded-xl border border-white/10 transition-colors ${editingTarget ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#0a2e1f]'}`}>
                    {editingTarget && <div className="text-xs text-blue-400 font-bold flex justify-between"><span>正在編輯：{editingTarget}</span><button onClick={cancelEdit}><X size={14}/></button></div>}
                    ...
                    <div className="flex gap-2">
                        <input value={inputName} onChange={e => setInputName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmitParticipant()} placeholder={editingTarget ? "修改名字..." : "輸入名字..."} className="flex-1 bg-[#031811] border border-white/20 rounded-lg px-4 py-2 outline-none focus:border-emerald-500"/>
                        <button onClick={handleSubmitParticipant} className={`px-4 py-2 rounded-lg text-white font-bold flex items-center gap-1 ${editingTarget ? 'bg-blue-600 hover:bg-blue-500' : 'bg-emerald-600 hover:bg-emerald-500'}`}>{editingTarget ? <RefreshCw size={16}/> : <Plus size={16}/>}{editingTarget ? '更新' : ''}</button>
                    </div>
                 </div>
                 ...
                 <div className="bg-[#0a2e1f] rounded-xl border border-white/10 overflow-hidden">
                    ...
                    <div className="divide-y divide-white/5">
                        {currentEvent.participants.map((name, i) => {
                            ...
                            return (
                                <div key={i} className={`flex justify-between items-center p-3 hover:bg-white/5 ${editingTarget === name ? 'bg-blue-900/20' : ''}`}>
                                    ...
                                    <div className="flex gap-1">
                                        <button onClick={() => handleEditParticipant(name, details)} className={`p-2 rounded ${editingTarget === name ? 'text-blue-300 bg-blue-900/50' : 'text-gray-500 hover:text-white'}`} title="編輯此人"><Pencil size={14}/></button>
                                        <button onClick={() => { if (!isFav) saveFavorites([...favorites, {name, ...details}]) }} className={`p-2 ${isFav ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-400'}`}><Star size={16} className={isFav ? 'fill-yellow-400' : ''}/></button>
              ...
              <div className="bg-[#0a2e1f] rounded-xl border border-white/10 overflow-hidden p-4 grid gap-2">
                 {favorites.map(fav => (
                     <div key={fav.name} className="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/5">
                         <div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${LEVEL_COLORS[fav.level] || LEVEL_COLORS[PlayerLevel.INTERMEDIATE]} text-white`}>{fav.name.slice(0,1)}</div><div><div className="text-gray-200 font-bold">{fav.name}</div><div className="text-[10px] text-gray-500">BP: {fav.battlePower}</div></div></div>
                         <div className="flex gap-1">
                             <button onClick={() => setEditingFavorite(fav)} className="p-1.5 text-blue-400 hover:bg-white/10 rounded"><Pencil size={14}/></button>
       ...
       {editingFavorite && (
           <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
               <div className="bg-[#062c1f] w-full max-w-sm rounded-2xl border border-emerald-500/50 p-6 shadow-2xl">
